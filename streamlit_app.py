...
# ---------------------------------------------------------------- 1. speaker
st.subheader("1 · Speaker")
gender = "M" if st.radio("Sex", ["Female", "Male"], horizontal=True,
                         label_visibility="collapsed") == "Male" else "F"
...
st.subheader("3 · Sentence — in German")
...
with st.expander("Hear the sentence", expanded=True):
    st.audio(str(HERE / f"ref_phrase_{gender}.wav"))
...
def get_audio():
    if example.startswith("Example: woman aged 21"):
        return ((HERE / "example_young_vowel.wav").read_bytes(),
                (HERE / "example_young_phrase.wav").read_bytes(), "F", 21)
    if example.startswith("Example: woman aged 76"):
        return ((HERE / "example_old_vowel.wav").read_bytes(),
                (HERE / "example_old_phrase.wav").read_bytes(), "F", 76)
    v = vowel_up or vowel
    p = phrase_up or phrase
    if v is None or p is None:
        return None
    return v.getvalue(), p.getvalue(), gender, None
...
if go:
    ...
    if got is None:
        ...
        v_bytes, p_bytes, sex, true_age = got
        with st.spinner("Measuring the voice…"):
            try:
                res = M.analyse(sex, v_bytes, p_bytes)
